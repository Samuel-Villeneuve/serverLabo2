...
module.exports = class BookmarksController extends require("./Controller") {
  ...
  queryStringParamsList() {
    ...
    content +=
      "<h4>? sort=key <br> return all bookmarks sorted by key values (Id, Name, Category, Url)";
    content +=
      "<h4>? sort=key,desc <br> return all bookmarks sorted by descending key values";
    content +=
      "<h4>? key=value <br> return the bookmark with key value = value";
    content +=
      "<h4>? key=value* <br> return the bookmark with key value that start with value";
    content +=
      "<h4>? key=*value* <br> return the bookmark with key value that contains value";
    content +=
      "<h4>? key=*value <br> return the bookmark with key value end with value";
    content +=
      "<h4>page?limit=int&offset=int <br> return limit bookmarks of page offset";
    content += "</div>";
    return content;
  }
  queryStringHelp() {
  ...
  post(bookmark) {
    ...
    if (Bookmark.valid(bookmark)) {
      ...
      if (
        this.bookmarksRepository.findByField("Name", bookmark.Name) !== null
      ) {
        this.response.conflict();
      } else {
        let newBookmark = this.bookmarksRepository.add(bookmark);
        if (newBookmark) this.response.created(newBookmark);
  ...
  put(bookmark) {
    ...
    if (Bookmark.valid(bookmark)) {
      let foundBookmark = this.bookmarksRepository.findByField(
        "Name",
        bookmark.Name
      );
